...
function saveSelectMemory(sels: readonly vscode.Selection[], args: SelectMemoryArgs,
...
function swapWithMemoryFn(editor: vscode.TextEditor,
    current: readonly vscode.Selection[], old: vscode.Selection[]){
...
function deletePrimary(){
    ...
    if(editor && editor.selections.length > 1){
        if(editor.selections.length > 1){
            let sels = <vscode.Selection[]>cloneDeep(editor.selections);
